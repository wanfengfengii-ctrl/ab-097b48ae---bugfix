...
- 规模友好：按主体名建桶（封存时用零密码学解析从 DER 抽取 Name 建索引），
  ...
  不会反复全量解析 DER。撤销证据同理：封存时用零密码学扫描抽取作用域
  头（CRL 的颁发者/AKI/IDP/delta 编号、OCSP 的 CertID 序列号）写入
  作用域侧车；进程重启、切换实例或裁决缓存未命中后，只有与可达证书
  作用域相关的撤销 DER 才被完整解析，无关 CRL/OCSP 不读取、不物化。
...
- `der/certificates|crls|ocsps/<sha256>.der`：路径/拒绝证明/策略与撤销
  实际引用的全部原始 DER。证书包含路径搜索可达的证书；撤销证据采用
  **惰性物化**——只有与可达证书的颁发者、AKI、分发点或 OCSP CertID
  序列号相关的 CRL/OCSP 才会被完整解析并打入包中，封存集里与当前裁决
  无作用域关系的撤销 DER（如无关 CA 的数十份 CRL）既不会被进程读取、
  解析，也不会进入证据包。包内 `manifest.json` 仍枚举封存集的完整
  内容清单（含未打包对象的指纹），因此内容摘要与完整清单仍可独立复核；
